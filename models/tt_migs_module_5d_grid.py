...
class TTUltraMIGSModule5DGrid(nn.Module):
    ...
    def init_from_tensor(self, gaussian_model):
        with torch.no_grad():
            ...
            # STEP 2: Bounding box
            # ─────────────────────────────────────────────────────
            coord_min = xyz.min(0).values
            coord_max = xyz.max(0).values
            ...
            min_res = self.cfg.migs.get("grid_min_res", None)
            max_res = self.cfg.migs.get("grid_max_res", None)
            min_res = int(min_res) if min_res is not None else None
            max_res = int(max_res) if max_res is not None else None

            grid_mode = str(self.cfg.migs.get("grid_mode", "budget")).lower()
            ...
            elif grid_mode in ["voxel_size", "voxel", "vs"]:
                v = self.cfg.migs.get("grid_voxel_size", 0.0232)
                if isinstance(v, (list, tuple, ListConfig)):
                    vx, vy, vz = map(float, list(v))
                else:
                    vx = vy = vz = float(v)

                vx = max(vx, 1e-9); vy = max(vy, 1e-9); vz = max(vz, 1e-9)

                Nx = int(math.ceil(Lx / vx)) + 1
                Ny = int(math.ceil(Ly / vy)) + 1
                Nz = int(math.ceil(Lz / vz)) + 1

                # clamp seulement si défini dans le YAML
                if min_res is not None:
                    Nx = max(Nx, min_res); Ny = max(Ny, min_res); Nz = max(Nz, min_res)
                if max_res is not None:
                    Nx = min(Nx, max_res); Ny = min(Ny, max_res); Nz = min(Nz, max_res)

            else:
                # mode budget (avec défaut si pas fourni)
                B = int(self.cfg.migs.get("grid_budget", 96_000))
                # ici, si tu enlèves min/max du YAML, _compute_grid_resolution_A2 doit recevoir des ints
                # donc on lui donne des valeurs "safe" locales
                _min = min_res if (min_res is not None) else 2
                _max = max_res if (max_res is not None) else 10**9
                Nx, Ny, Nz = self._compute_grid_resolution_A2(Lx, Ly, Lz, B, min_res=_min, max_res=_max)

            print(f"[TT-Grid] grid_mode={grid_mode}, grid_voxel_size={self.cfg.migs.get('grid_voxel_size', None)}")
            print(f"[TT-Grid] Resolution: ({Nx}, {Ny}, {Nz}) = {Nx*Ny*Nz} voxels")
            dx = Lx / max(Nx - 1, 1)
            dy = Ly / max(Ny - 1, 1)
            dz = Lz / max(Nz - 1, 1)
            ...
            # ─────────────────────────────────────────────────────
            xyz_norm = (xyz - coord_min) / (coord_max - coord_min)
    ...
    def get_W_for_identity(self, idx: int, xyz_query: torch.Tensor = None) -> torch.Tensor:
        ...
        # Normalize to [-1, 1]
        xyz_norm = (xyz_query - self.grid_coord_min) / (self.grid_coord_max - self.grid_coord_min)
