...
class TTUltraMIGSModule4DUVGridNoOpacity(nn.Module):
    """
    Tensor-Train MIGS on a GEOMETRIC UV GRID — M=42.
    opacity is EXCLUDED from TT (kept explicit in scene.py).
    xyz IS compressed inside the TT.

    Factorizes a tensor: (I, Nu, Nv, M)
      - I   : identities
      - Nu  : U grid resolution
      - Nv  : V grid resolution
      - M   : 42 — xyz+scaling+rotation+dc+rest only

    Parameter layout in W (M=42):
      cols 0:3   -> xyz      (3)
      cols 3:6   -> scaling  (3)
    ...
    "core4 trick":
      ...
      core1 : (r1, Nu, r2)
      core2 : (r2, Nv, r3)
      core4 : (r3, 42, 1)   split into per-block nn.Parameters

    LR policy (reads from cfg.migs YAML):
      position_lr_init / position_lr_final / position_lr_max_steps
        -> LambdaLR decay on core0, core1, core2, core4_xyz
    ...
    def __init__(self, cfg):
        ...
        # core4 split — M=42: xyz(3)+scaling(3)+rotation(4)+dc(1)+rest(31)
    ...
    def _to_tt_ranks_4d(self, rank):
        ...
        if isinstance(rank, (list, tuple)):
            ...
            assert len(rank) == 5, f"[TT-UV-NoOp] rank list must be length 5, got {rank}"
            return rank
    ...
    def _zero_pad_pair_preserve(self, left, right, add, dim_left, dim_right):
        ...
        std_l = left.detach().std();  std_r = right.detach().std()
    ...
    def _expand_ranks_to_targets_preserve(self, rank_or_ranks):
        ...
        if rt[3] > c2.shape[2]:
            ...
            self.tt_tensor_gpu[2] = nn.Parameter(new_c2)
            assert new_core4.shape[1] == 42, f"[TT-UV-NoOp] core4 M={new_core4.shape[1]} != 42"
    ...
    def _debug_print_core_shapes(self, tag=""):
        if len(self.tt_tensor_gpu) < 3:
            print(f"[TT-UV-NoOp]{tag} cores not yet initialized"); return
        c4 = self.recombine_core4()
        print(f"[TT-UV-NoOp]{tag} target ranks = {self.tt_rank}")
        print(f"[TT-UV-NoOp]{tag} core0 : {tuple(self.tt_tensor_gpu[0].shape)}")
        print(f"[TT-UV-NoOp]{tag} core1 : {tuple(self.tt_tensor_gpu[1].shape)}")
        print(f"[TT-UV-NoOp]{tag} core2 : {tuple(self.tt_tensor_gpu[2].shape)}")
        print(f"[TT-UV-NoOp]{tag} core4 : {tuple(c4.shape)}  # (r3,42,1)")

    # ------------------------------------------------------------------ init --

    def init_from_tensor(self, gaussian_model):
        """
        Build TT from UV grid — M=42 (xyz, scaling, rotation, dc, rest).
        opacity is NOT included in the TT.
        """
        with torch.no_grad():
            if hasattr(self.cfg, "migs") and getattr(self.cfg.migs, "skip_init_from_tensor", False):
                print("[TT-UV-NoOp] skip_init_from_tensor=True — skipping"); return

            device = gaussian_model._xyz.device
            G      = gaussian_model._xyz.shape[0]
            print(f"[TT-UV-NoOp] Initializing from {G} Gaussians (M=42, opacity explicit)")

            # ---- Assemble W (G, 42) — NO opacity ----
            ...
            W_GM = torch.cat([xyz, scaling, rotation, features_dc, features_rest], dim=1)
            M    = W_GM.shape[1]
            assert M == 42, f"[TT-UV-NoOp] Expected M=42, got {M}"
            ...
            if not (hasattr(gaussian_model, "_uv") and gaussian_model._uv is not None):
                raise RuntimeError("[TT-UV-NoOp] gaussian_model has no _uv attribute.")
            ...
            print(f"[TT-UV-NoOp] Resolution: (Nu,Nv)=({Nu},{Nv})  cells={Nu*Nv}")
            ...
            print(f"[TT-UV-NoOp] Bleed: {n_bleed} iters — "
                  f"occupied {occupied.sum().item()}/{Nu*Nv} "
                  f"({100*occupied.sum().item()/(Nu*Nv):.1f}%)")

            still_empty = ~occupied; n_still = int(still_empty.sum().item())
            if n_still > 0:
                grid[0][still_empty] = W_prior.unsqueeze(0).expand(n_still,-1)
                print(f"[TT-UV-NoOp] {n_still} cells still empty -> W_prior")
            ...
            print(f"[TT-UV-NoOp] Confidence: min={conf.min():.3f} max={conf.max():.3f} mean={conf.mean():.3f}")

            # ---- TT decomposition (1, Nu, Nv, 42) ----
            ...
            print(f"[TT-UV-NoOp] Init complete. ranks={self.tt_rank}  tensor=(I=1,Nu={Nu},Nv={Nv},M=42)")
            self._needs_opt_rebuild = True
            self._grid_cache.clear()
    ...
    def _reconstruct_grid_for_identity(self, identity_idx: int) -> torch.Tensor:
        return tt_to_tensor(self.get_tt_tensor(identity_idx))  # (1,Nu,Nv,42)
    ...
    def get_W_for_identity(self, identity_idx: int, uv_query: torch.Tensor = None) -> torch.Tensor:
        ...
        if not hasattr(self, "NuNv"):
            raise RuntimeError("[TT-UV-NoOp] Call init_from_tensor before sampling.")
        if uv_query is None:
            if not hasattr(self, "gaussian_uv"):
                raise RuntimeError("[TT-UV-NoOp] No uv_query and no stored gaussian_uv.")
        ...
        inp  = grid_full.permute(2,1,0).unsqueeze(0).contiguous()  # (1,42,Nv,Nu)
        ...
        if mode == "all_nearest":
            sampled = _gs(inp, "nearest").view(42,-1).permute(1,0).contiguous()
        else:
            sampled = _gs(inp, "bilinear").view(42,-1).permute(1,0).contiguous()

        if self.verbose:
            print(f"[TT-UV-NoOp] mode={mode} | "
    ...
    def expand_first_core(self, n_identities: int):
        if len(self.tt_tensor_gpu) == 0:
            raise RuntimeError("[TT-UV-NoOp] TT cores must be initialized first.")
    ...
    def add_identity(self, noise_scale: float = 0.05, rebuild_optimizer: bool = True) -> int:
        if len(self.tt_tensor_gpu) == 0:
            raise RuntimeError("[TT-UV-NoOp] TT cores not initialized.")
        core0 = self.tt_tensor_gpu[0]; _, I, _ = core0.shape
        U = core0.detach()
    ...
    def set_optimizer(self, opt_cfg):
        """
        Decayed LR (position_lr_*): core0, core1, core2, core4_xyz
        Fixed LR: core4_scaling, core4_rotation, core4_dc, core4_rest
        opacity handled by explicit_optimizer in scene.py.
        """
        ...
        # Decayed: core4_xyz (position-like)
        param_groups.append({"params":[self.core4_xyz], "lr":lr_init, "initial_lr":lr_init, "final_lr":lr_final})
        decayed_idx.append(len(param_groups)-1)

        # Fixed LR: other core4 blocks
        param_groups += [
        ...
        self._needs_opt_rebuild = False
        print(f"[TT-UV-NoOp] Optimizer built — lr_init={lr_init:.2e} lr_final={lr_final:.2e} "
    ...
    def step(self, iteration=None):
        ...
        if iteration is not None and not self._tt_unfrozen and iteration >= self.tt_delay:
            ...
            if self.verbose:
                print(f"[TT-UV-NoOp] Parameters unfrozen at iteration {iteration}")
    ...
    def compute_tv_loss(self, identity_idx: int) -> torch.Tensor:
        """TV on xyz channel (cols 0:3) since opacity is not in TT."""
        grid   = self._reconstruct_grid_for_identity(identity_idx)  # (1,Nu,Nv,42)
        xyz_g  = grid[:, :, :, 0:3]
        diff_u = xyz_g[:, 1:, :, :] - xyz_g[:, :-1, :, :]
        diff_v = xyz_g[:, :, 1:, :] - xyz_g[:, :, :-1, :]
