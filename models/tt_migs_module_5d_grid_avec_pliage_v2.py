...
class TTUltraMIGSModule5DGrid(nn.Module):
    ...
    def _compute_grid_resolution_from_voxel_size(self, Lx, Ly, Lz, dx, dy, dz, min_res=8, max_res=256):
        Lx = float(max(Lx, 1e-12)); Ly = float(max(Ly, 1e-12)); Lz = float(max(Lz, 1e-12))
        dx = float(max(dx, 1e-12)); dy = float(max(dy, 1e-12)); dz = float(max(dz, 1e-12))

        Nx = int(math.ceil(Lx / dx)) + 1
        Ny = int(math.ceil(Ly / dy)) + 1
        Nz = int(math.ceil(Lz / dz)) + 1

        Nx = max(min_res, min(Nx, max_res))
        Ny = max(min_res, min(Ny, max_res))
        Nz = max(min_res, min(Nz, max_res))
        return Nx, Ny, Nz
    ...
    def init_from_tensor(self, gaussian_model):
        with torch.no_grad():
            ...
            dx0 = dy0 = dz0 = None
            if grid_mode in ["voxel_size", "voxelsize", "fixed_voxel", "fixed_voxel_size"]:
                v = self.cfg.migs.get("grid_voxel_size", None)
                if v is None:
                    raise ValueError("grid_mode=voxel_size requires migs.grid_voxel_size = [dx,dy,dz].")
                dx0, dy0, dz0 = map(float, list(v))
            ...
            elif grid_mode in ["voxel_size", "voxelsize", "fixed_voxel", "fixed_voxel_size"]:
                v = self.cfg.migs.get("grid_voxel_size", None)
                if v is None:
                    raise ValueError("grid_mode=voxel_size requires migs.grid_voxel_size = [dx,dy,dz] (from no-fold run).")

                if isinstance(v, (list, tuple, ListConfig)) and len(v) == 3:
                    dx0, dy0, dz0 = map(float, list(v))
                else:
                    raise ValueError("grid_voxel_size must be a list of 3 floats: [dx, dy, dz].")

                Nx, Ny, Nz = self._compute_grid_resolution_from_voxel_size(
                    Lx, Ly, Lz, dx0, dy0, dz0, min_res=min_res, max_res=max_res
                )

            else:
                # budget mode (ton comportement actuel)
