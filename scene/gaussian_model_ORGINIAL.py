...
class GaussianModel:
    ...
    def create_from_pcd(self, pcd : BasicPointCloud, spatial_lr_scale=1.):
        ...
        #print("**************** initial opacity ", self._opacity)
    ...
    # def setup_xyz_optimizer(self, lr=1.6e-4):
    #     # lr petit, stable
    #     self.xyz_optimizer = torch.optim.Adam([self._xyz], lr=lr)
    ...
    def save_ply_playcanvas(self, path, rgb=None, fill_gray=0.5):
        ...
        # --- Extra features (f_rest) set to zero ---
        f_rest45 = np.zeros((N, 45), dtype=np.float32)

        # --- Header definition in the expected ORDER ---
        prop_names = [
            'x','y','z',
        ...
        # --- Concatenate attributes exactly in order ---
